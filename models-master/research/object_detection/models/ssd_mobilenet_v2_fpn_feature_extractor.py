...
def _create_modified_mobilenet_config():
  ...
  conv_defs['spec'][-1] = mobilenet.op(
      slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=256) #256
  return conv_defs
    ...
    with tf.variable_scope('MobilenetV2', reuse=self._reuse_weights) as scope:
      ...
      with slim.arg_scope(self._conv_hyperparams_fn()):
        with tf.variable_scope('fpn', reuse=self._reuse_weights):
            # Dodatkowe wyjścia ze zmodyfikowanej sieci MobileNet V2: layer_21, layer_23, layer_25
          feature_blocks = [
              'layer_4', 'layer_7', 'layer_14', 'layer_19', 'layer_21', 'layer_23', 'layer_25'
          ]
          base_fpn_max_level = min(self._fpn_max_level, 8)
          feature_block_list = []
